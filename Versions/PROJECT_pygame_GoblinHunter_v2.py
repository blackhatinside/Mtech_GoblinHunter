...
class Player(object):
	...
	def hit(self, damage):		# when player gets hit
		...
		self.posy = 400
		self.walkCount = 0
		font1 = pygame.font.SysFont('comicsans', 64, True)
		text = font1.render(random.choice(messages_list), 1, Color.RED.value)
...
class Enemy(object):
	...
	def draw(self, win):
		...
		if self.visible:
			...
			pygame.draw.rect(win, Color.DARKGREEN.value, (self.hitbox[0], self.hitbox[1] - 20, 50 - (5 * (10 - self.health)), 10))
			self.hitbox = (self.posx + 17, self.posy + 2, 40, 55)
		# pygame.draw.rect(win, (255, 0, 0), self.hitbox, 2)
...
def redrawGameWindow():
	...
	text = font.render(f'Score: {score}', 1, Color.BLACK.value)
